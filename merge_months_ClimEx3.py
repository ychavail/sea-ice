...
for sim in simulations:
    ...
    lon_max     = [-56.]#-74.,-68.,-62.,-56.,-74.,-68.,-62.,-56.,-74.,-68.,-62.,-56.,-74.,-68.,-62.,
    lat_min     = [45.]#,45.,45.,45.,49.5,49.5,49.5,49.5,54.,54.,54.,54.,58.5,58.5,58.5,58.5]
    lat_max     = [63.]#49.5,49.5,49.5,49.5,54.,54.,54.,54.,58.5,58.5,58.5,58.5,63.,63.,63.,

    for s in range(1):
        dsSub   = dataset.where((dataset.lon>=lon_min[s])&(dataset.lon<lon_max[s])&(dataset.lat>=lat_min[s])&(dataset.lat<lat_max[s]),drop=True)
        tasmax  = dsSub['tasmax'][:,:,:]
        tasmax.to_netcdf(('/exec/yanncha/sea_ice/tasmax/tasmax_rearranged_'+sim+'_sqall.nc'))
