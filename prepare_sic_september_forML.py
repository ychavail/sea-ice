##################################################################
# Description: Prepare september sic data for machine learning operations: len(sim_family) x len(fam_member) x len(years) vectors of lat x lon size representing mean annual sic in the 60N-90N band.
# Code name: prepare_sic_september_forML.py
# Date of creation: 2018/05/01
# Date of last modification: 2018/07/27
...
for f in sim_family:
	for m in fam_member:
		...
		for y1 in years1:
			ind_t		= fct_t.selmonthyear(y1,9,time1[:],time1.units,time1.calendar)
		...
		for y2 in years2:
			ind_t		= fct_t.selmonthyear(y2,9,time2[:],time2.units,time2.calendar)
...
outputdir	= '/exec/yanncha/sea_ice/'
outputfilename	= (outputdir+'sic_september_forML_CanESM2-LE.nc')
...
outputfile.description	= "Sea ice fraction in the Northern Hemisphere (60N-90N) on average for September from the CanESM2-LE."
